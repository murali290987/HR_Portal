...
- **`known_limitation` cases**: reported completely separately in every script's output, and excluded from every pass/fail gate in `test_regressions.py`. Currently two, both stemming from the same root gap (the guardrail's `EMP\d+` regex never catches a name or a bare pronoun) reached via two different paths — `known_limitation_name_bypass` (a real owner asking about a different person *by name*) and `known_limitation_hr_first_person_bypass` (the `hr` role's ownership bypass plus a vague "my ..." question with no name or ID at all) — the point is to *see* them fail on every run, not to silently patch the dataset until they stop.
