...
|---|---|---|---|
| `dataset.py` | 21 hand-built test cases across 7 categories, grounded in `hr_docs/`'s actual content. Not a script you run — imported by everything below. | No | N/A |
...
| `FINDINGS.md` | Not a script — a record of what the sweep actually found (winning config, the table that justified it) and other investigation results, including negative ones. Not a script you run. | N/A | Read before re-running the sweep or re-litigating a chunking decision. |
...
- **`known_limitation` cases**: reported completely separately in every script's output, and excluded from every pass/fail gate in `test_regressions.py`. Currently one: `known_limitation_name_bypass` (a real owner asking about a different person *by name* — the guardrail's `EMP\d+` regex never catches a name). A second, related case — the `hr` role's ownership bypass plus a vague "my ..." question with no name or ID at all — was found the same way and has since been **fixed** (in `retrieve()`, not the guardrail) and promoted to a real regression gate, `hr_first_person_no_subject_blocked`; see `evals/FINDINGS.md`. The point of a `known_limitation` case is to *see* it fail on every run, not to silently patch the dataset until it stops — but once a fix genuinely lands, it graduates out of this bucket, as that one did.
